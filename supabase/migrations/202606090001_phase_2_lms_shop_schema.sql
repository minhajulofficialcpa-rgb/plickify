-- Phase 2: LMS + digital shop database schema for Supabase.
-- Database-only migration. No frontend or payment UI is implemented here.
...
after insert or update of activation_status on public.enrollments
...
create policy profiles_own_or_admin on public.profiles for all using (id = auth.uid() or public.has_admin_role()) with check (id = auth.uid() or public.has_admin_role());
...
create policy courses_read on public.courses for select using (status = 'published' or public.has_admin_role());
...
create policy batches_read on public.batches for select using (status in ('enrolling', 'active', 'completed') or public.has_admin_role());
...
create policy lessons_read on public.course_lessons for select using (is_preview or status = 'published' or public.has_admin_role());
create policy lessons_admin_write on public.course_lessons for all using (public.has_admin_role('instructor')) with check (public.has_admin_role('instructor'));
create policy products_read on public.products for select using (status = 'published' or public.has_admin_role());
...
create policy orders_owner_or_admin on public.orders for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy enrollments_owner_or_admin on public.enrollments for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy user_batches_owner_or_admin on public.user_batches for select using (user_id = auth.uid() or public.has_admin_role());
create policy payments_owner_or_admin on public.payments for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy payment_webhook_logs_admin on public.payment_webhook_logs for all using (public.has_admin_role()) with check (public.has_admin_role());
create policy downloads_owner_or_admin on public.downloads for select using (user_id = auth.uid() or public.has_admin_role());
...
create policy submissions_owner_or_instructor on public.assignment_submissions for all using (user_id = auth.uid() or public.has_admin_role('instructor')) with check (user_id = auth.uid() or public.has_admin_role('instructor'));
create policy watch_analytics_owner_or_admin on public.watch_analytics for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy support_tickets_participant_or_support on public.support_tickets for all using (user_id = auth.uid() or assigned_to = auth.uid() or public.has_admin_role('support')) with check (user_id = auth.uid() or assigned_to = auth.uid() or public.has_admin_role('support'));
create policy support_messages_participant_or_support on public.support_messages for all using (public.has_admin_role('support') or exists (select 1 from public.support_tickets st where st.id = ticket_id and st.user_id = auth.uid())) with check (sender_id = auth.uid() or public.has_admin_role('support'));
create policy certificates_owner_or_admin on public.certificates for select using (user_id = auth.uid() or public.has_admin_role());
create policy invoices_owner_or_admin on public.invoices for select using (user_id = auth.uid() or public.has_admin_role());
create policy contact_messages_support on public.contact_messages for all using (public.has_admin_role('support')) with check (public.has_admin_role('support'));
create policy reviews_owner_public_or_admin on public.reviews for all using (is_published or user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy device_sessions_owner_or_admin on public.device_sessions for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
create policy notifications_owner_or_admin on public.notifications for all using (user_id = auth.uid() or public.has_admin_role()) with check (user_id = auth.uid() or public.has_admin_role());
