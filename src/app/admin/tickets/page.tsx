      ...
      <div className="mt-8">
        <AdminSection title="Tickets">
          <AdminTable rows={tickets} emptyLabel="No support tickets found." columns={[
            ...
            { header: "Thread", cell: (row) => <div className="grid max-w-sm gap-2">{(row.support_messages ?? []).map((message) => <div key={message.id} className="rounded-[0.75rem] border border-white/10 bg-white/[0.04] p-2"><p className="text-xs font-bold text-white">{message.sender_role ?? "student"}</p><p className="whitespace-pre-wrap text-xs text-muted-foreground">{message.message}</p>{message.attachment_url ? <Link href={message.attachment_url} className="text-xs text-accent">Attachment</Link> : null}</div>)}</div> },
