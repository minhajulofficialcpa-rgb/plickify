...
export interface AdminEnrollmentRow { id: string; user_id: string; course_id: string; batch_id: string | null; status?: string | null; activation_status: string | null; created_at: string; profiles?: Relation<Pick<AdminUserRow, "full_name" | "email">>; courses?: Relation<Pick<CourseSummary, "title">>; batches?: Relation<Pick<BatchSummary, "title">> }
export interface AdminProductRow { id: string; title: string; slug: string; category: string | null; price_bdt: number | null; access_type: string | null; private_file_path: string | null; download_bucket: string | null; status: string | null; created_at: string }
export interface AdminOrderRow { id: string; order_number: string | null; user_id: string | null; status: string; payment_status: string | null; activation_status: string | null; access_type: string | null; item_type: string | null; total_bdt: number | null; created_at: string; profiles?: Relation<Pick<AdminUserRow, "full_name" | "email">>; products?: Relation<Pick<AdminProductRow, "title" | "category">> }
...
export interface AdminContactRow { id: string; full_name: string | null; name?: string | null; email: string | null; subject: string | null; status?: string | null; created_at: string }
...
export async function getAdminAnalytics(): Promise<AdminMetric[]> {
  ...
  const [students, activeStudents, pendingOrders, successfulPayments, activeEnrollments, paidOrders, submissions, assignments, completedLessons, watchRows, openTickets, resolvedTickets] = await Promise.all([
    ...
    readCount(supabase.from("enrollments").select("id", { count: "exact", head: true }).eq("activation_status", "active")),
...
export async function getAdminProducts() { if (!canUseAdmin()) return empty<AdminProductRow>(); return readList<AdminProductRow>(createAdminClient().from("products").select("id, title, slug, category, price_bdt, access_type, private_file_path, download_bucket, status, created_at").order("created_at", { ascending: false }).limit(100)); }
export async function getAdminOrders() { if (!canUseAdmin()) return empty<AdminOrderRow>(); return readList<AdminOrderRow>(createAdminClient().from("orders").select("id, order_number, user_id, status, payment_status, activation_status, access_type, item_type, total_bdt, created_at, profiles(full_name, email), products(title, category)").order("created_at", { ascending: false }).limit(100)); }
export async function getAdminEnrollments() { if (!canUseAdmin()) return empty<AdminEnrollmentRow>(); return readList<AdminEnrollmentRow>(createAdminClient().from("enrollments").select("id, user_id, course_id, batch_id, activation_status, created_at, profiles(full_name, email), courses(title), batches(title)").order("created_at", { ascending: false }).limit(100)); }
...
export async function getAdminContacts() { if (!canUseAdmin()) return empty<AdminContactRow>(); const rows = await readList<AdminContactRow>(createAdminClient().from("contact_messages").select("id, name, email, subject, created_at").order("created_at", { ascending: false }).limit(100)); return rows.map((row) => ({ ...row, full_name: row.full_name ?? row.name ?? null, status: row.status ?? "new" })); }
...
export async function getAdminAssignments() {
  ...
  const [assignments, submissions] = await Promise.all([
    ...
    readList<AdminSubmissionRow>(supabase.from("assignment_submissions").select("id, assignment_id, user_id, status, marks, score, feedback, submission_text, submission_url, github_url, attachment_url, submitted_at, assignments(title)").order("submitted_at", { ascending: false }).limit(100))
  ]);
